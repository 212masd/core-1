// ==UserScript==
// @name         B站多关键词抓取·6关键词版（可滚动+导出TXT+自选路径）
// @version      12.0
// @match        *://*/*
...
(function() {
    ...
    taskList.style.maxHeight = "220px";
    ...
    tasks.push(createTask(3));
    tasks.push(createTask(4));
    tasks.push(createTask(5));
    ...
    exportBtn.textContent = "导出TXT（可自选位置）";
    exportBtn.style.cssText = "width:100%; padding:8px; background:#1e87f0; color:#fff; border:none; border-radius:4px; cursor:pointer; margin-top:6px;";
    ...
    exportBtn.onclick = () => {
        ...
        a.href = url;
        a.download = "B站视频链接.txt";
        a.click();
        URL.revokeObjectURL(url);

        alert("已复制链接并开始下载\n你可以新建文本文档 → 粘贴 → 保存到任意位置");
    };
